...
def combine_split_files():
    ...
    # SILENT: Don't show file checking messages
    if missing_parts:
        st.error("❌ Missing model parts. Please check file uploads.")
        return False
    ...
    try:
        ...
        # SILENT: No success messages
        return True
        
    except Exception as e:
        st.error(f"❌ Error combining model files")
        return False

# Cache resources
@st.cache_resource
def load_artifacts():
    """Load all model artifacts (SILENT VERSION)"""
    # Combine split files first (silently)
    ...
    try:
        ...
        st.error(f"❌ Error loading AI models")
        return None, None, None, None
...
# Main UI - Clean without file combination messages
