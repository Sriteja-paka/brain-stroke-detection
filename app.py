# ===== STREAMLIT APP WITH WORKING GRAD-CAM & SHAP =====
...
class BrainStrokeDetector:
    ...
    def generate_grad_cam(self, processed_img):
        """Generate Grad-CAM using feature extractor models instead of main model"""
        try:
            # Use the EfficientNetV2S feature extractor for Grad-CAM (it has proper conv layers)
            feature_model = self.feature_models['effnet_s']
            
            # Find convolutional layers in the feature extractor
            conv_layers = []
            ...
            with tf.GradientTape() as tape:
                ...
                # Use the mean of predictions as target (since we don't have class-specific output here)
    ...
    def generate_shap_explanation(self, selected_features):
        """Generate proper SHAP explanation with bar graphs"""
        try:
            if not SHAP_AVAILABLE:
                return None, None
            
            # Create a simpler SHAP explanation using KernelExplainer
            background = np.random.rand(50, selected_features.shape[1]) * 0.1  # Small background
            
            def predict_fn(x):
                # Scale the features first
                x_scaled = self.scaler.transform(x) if x.shape[1] == self.scaler.n_features_in_ else x
                x_selected = x_scaled[:, self.best_mask] if x.shape[1] == self.scaler.n_features_in_ else x
                return self.ultimate_model_gwo.predict(x_selected, verbose=0)
            
            # Use KernelExplainer
            explainer = shap.KernelExplainer(predict_fn, background)
            
            # Calculate SHAP values for our single instance
            shap_values = explainer.shap_values(selected_features, nsamples=50)
            
            # Get the SHAP values for the stroke class (class 1)
    ...
    def predict_image(self, processed_img, original_img=None):
        ...
        try:
            ...
            # Generate Grad-CAM using feature extractor
