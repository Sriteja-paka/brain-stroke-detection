...
def combine_split_files():
    """Combine the 7 specific part files (COMPLETELY SILENT)"""
    ...
    # Check if all parts exist (silently)
    for part in expected_parts:
        if not os.path.exists(part):
            return False
    
    # Combine the parts (completely silent)
    try:
...
@st.cache_resource
def load_artifacts():
    """Load all model artifacts (silent)"""
