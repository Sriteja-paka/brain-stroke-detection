...
def combine_split_files():
    """Combine split model files into original file"""
    ...
    # Find all parts
    parts = sorted([f for f in os.listdir('.') if f.startswith('feature_extractor.keras.part')])
    
    if not parts:
        st.error("❌ No model parts found! Please upload all feature_extractor.keras.partXXX files")
        return False
    
    st.info(f"🔄 Combining {len(parts)} model parts...")
    ...
    try:
        ...
        st.success("✅ Model combined successfully!")
        return True
