# ===== STREAMLIT APP - SIMPLE WORKING SHAP =====
...
class BrainStrokeDetector:
    ...
    def simple_feature_analysis(self, selected_features, prediction):
        """SIMPLE feature analysis without SHAP"""
        try:
            if selected_features is None or selected_features.size == 0:
                return None, None
            
            # Get feature importance using simple method
            # Use the model's weights to understand feature importance
            feature_importance = np.abs(selected_features[0])
            
            # Get top 10 features
            top_n = min(10, len(feature_importance))
            top_indices = np.argsort(feature_importance)[::-1][:top_n]
            top_features = [f"Feature_{i}" for i in top_indices]
            top_values = selected_features[0][top_indices]
            
            # Calculate impact direction based on prediction
            # If stroke probability is high, positive features support stroke
            stroke_prob = prediction[0][1]
            impact_direction = []
            
            for val in top_values:
                if stroke_prob > 0.5:
                    # For stroke prediction, positive values support stroke
                    impact_direction.append('Supports Stroke' if val > 0 else 'Against Stroke')
                else:
                    # For normal prediction, positive values support normal (against stroke)
                    impact_direction.append('Against Stroke' if val > 0 else 'Supports Stroke')
            
            # Calculate percentages
            total_impact = np.sum(np.abs(top_values))
            if total_impact > 0:
                percentages = [(abs(val) / total_impact) * 100 for val in top_values]
            else:
                percentages = [0] * len(top_values)
            
            # Create bar chart
            fig, ax = plt.subplots(figsize=(12, 8))
            y_pos = np.arange(len(top_features))
            
            # Color coding based on impact direction
            colors = []
            for impact in impact_direction:
                if impact == 'Supports Stroke':
                    colors.append('#2ecc71')  # Green
                else:
                    colors.append('#e74c3c')  # Red
            
            bars = ax.barh(y_pos, top_values, color=colors, alpha=0.8, height=0.7)
            ax.set_yticks(y_pos)
            ax.set_yticklabels(top_features, fontsize=10)
            ax.set_xlabel('Feature Value Impact', fontsize=12, fontweight='bold')
            ax.set_title('Top 10 Features Influencing Prediction', fontsize=14, fontweight='bold')
            ax.invert_yaxis()
            ...
            for i, (bar, value) in enumerate(zip(bars, top_values)):
                ...
                color = 'darkgreen' if colors[i] == '#2ecc71' else 'darkred'
            ...
            # Prepare table data
            table_data = []
            for i, (feature, value, percent, impact) in enumerate(zip(top_features, top_values, percentages, impact_direction)):
                table_data.append({
                    'Rank': i + 1,
                    'Feature': feature,
                    'Value': f"{value:.6f}",
    ...
    def predict_image(self, processed_img):
        """REAL PREDICTION with SIMPLE feature analysis"""
        try:
            if not self.models_loaded:
                return None
            ...
            # 5. Generate simple feature analysis
            feature_fig, feature_table_data = None, None
            status_text.text("📊 Analyzing features...")
            feature_fig, feature_table_data = self.simple_feature_analysis(selected_features, prediction)
...
def show_home_page():
    ...
    with col1:
        ...
        - **Feature Impact Analysis**: Understand AI decisions
            ...
            if img is not None:
                ...
                if processed_img is not None:
                    ...
                    if result is not None:
                        ...
                        if result['feature_fig'] is not None and result['feature_table_data'] is not None:
                            ...
                            st.table(result['feature_table_data'])
                            
                            # Interpretation
                            st.markdown("#### 💡 Interpretation:")
                            positive_features = [f for f in result['feature_table_data'] if f['Effect'] == 'Supports Stroke']
                            negative_features = [f for f in result['feature_table_data'] if f['Effect'] == 'Against Stroke']
                            ...
                            st.info("ℹ️ Feature analysis not available for this image")
