...
def combine_split_files():
    """Combine the 7 specific part files"""
    ...
    # Show what we found
    st.write(f"🔍 Looking for model parts...")
    st.write(f"✅ Found: {len(found_parts)} parts")
    st.write(f"❌ Missing: {len(missing_parts)} parts")
    
    if missing_parts:
        st.error("Missing parts: " + ", ".join(missing_parts))
        ...
        with open(target_file, 'wb') as outfile:
            for part in found_parts:
                with open(part, 'rb') as infile:
                    ...
                    part_size = len(content) / (1024 * 1024)
                    total_size += part_size
                    st.write(f"  ✅ Added {part} ({part_size:.1f} MB)")
        
        st.success(f"✅ Model combined! Total: {total_size:.1f} MB")
        return True
        
    except Exception as e:
        st.error(f"❌ Error combining: {e}")
        return False
