# ===== ENHANCED STREAMLIT APP WITH GRAD-CAM & CLAHE =====
...
class BrainStrokeDetector:
    ...
    def generate_grad_cam(self, model, img, layer_name="conv5_block32_2_conv"):
        ...
        try:
            ...
            superimposed_img = cv2.addWeighted(
                cv2.cvtColor(np.uint8(img * 255), cv2.COLOR_RGB2BGR), 0.6,
                heatmap, 0.4, 0
            )
...
def show_home_page():
    ...
    with col1:
        ...
        - **Grad-CAM Visualization**: See where the AI focuses its attention
        ...
        3. View detailed analysis and heatmaps
...
def show_analysis_page(detector):
    ...
    if not detector.load_models():
        st.error("Please ensure all model files are available in the current directory.")
        return
    ...
    if uploaded_file is not None:
        ...
        if img is not None:
            ...
            with st.spinner("🔄 Processing image and generating analysis..."):
                ...
                # Generate Grad-CAM
                grad_cam_img, heatmap = detector.generate_grad_cam(
                    detector.ultimate_model_gwo, 
                    processed_img[0]  # Remove batch dimension
                )
            ...
            if grad_cam_img is not None:
                ...
                with col6:
                    ...
                    st.image(grad_cam_img, use_column_width=True)
                
                st.info("""
                **💡 Grad-CAM Explanation:** 
                The red areas show where the AI model is focusing its attention. 
                Brighter red regions indicate higher importance in the decision-making process.
                """)
