# ===== STREAMLIT APP WITH OPTIMIZED PREDICTION =====
...
class BrainStrokeDetector:
    ...
    def build_optimized_feature_extractor(self):
        """Build SINGLE optimized feature extractor for speed"""
        try:
            with st.spinner("🔄 Building optimized feature extractor..."):
                IMG_SIZE = self.IMG_SIZE
                
                # Use only ONE model for speed - EfficientNetV2S
                ...
                self.feature_extractor = Model(inputs=inputs, outputs=pooled_features)
                
                st.success("✅ Optimized feature extractor built!")
                return True
                
        except Exception as e:
            st.error(f"❌ Error building feature extractor: {str(e)}")
            return False
    ...
    def extract_features_fast(self, processed_img):
        """FAST feature extraction with single model"""
        try:
            input_batch = np.expand_dims(processed_img, axis=0)
            
            # Use single optimized feature extractor
            features = self.feature_extractor.predict(input_batch, verbose=0)
    ...
    def load_models(self):
        """Load all required models - OPTIMIZED"""
        try:
            if not self.models_loaded:
                with st.spinner("🔄 Loading AI models... This may take a moment"):
                    ...
                    if not self.build_optimized_feature_extractor():
                        return False
    ...
    def predict_image_fast(self, processed_img):
        """ULTRA-FAST PREDICTION with minimal delays"""
        try:
            if not self.models_loaded:
            ...
            features = self.extract_features_fast(processed_img)
            ...
            # 5. Optional features (non-blocking)
...
def show_home_page():
    ...
    with col1:
        ...
        ### 🔬 Features:
        - **Real Feature Extraction**: Optimized for speed
...
def show_analysis_page():
    ...
    uploaded_file = st.file_uploader(
        ...
        key="file_uploader"  # Unique key to avoid duplicates
    )
        ...
        try:
            ...
            if img is not None:
                ...
                if processed_img is not None:
                    ...
                    # Make prediction
                    result = detector.predict_image_fast(processed_img)
...
def show_file_setup_page():
    ...
    - Single model feature extraction (3x faster)
    - No BiLSTM compatibility issues
...
def main():
    ...
    # Use unique keys for all widgets to avoid duplicates
