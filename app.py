# ===== STREAMLIT APP - CLEAN WORKING VERSION =====
...
class BrainStrokeDetector:
    ...
    def predict_image(self, processed_img):
        """REAL PREDICTION with debugging"""
        try:
            if not self.models_loaded:
                return None
            ...
            with st.expander("🔍 Debug Info"):
                ...
                st.write(f"Class 0 (Normal) probability: {normal_prob:.4f}")
                st.write(f"Class 1 (Stroke) probability: {stroke_prob:.4f}")
                st.write(f"Selected class: {final_class} (0=Normal, 1=Stroke)")
