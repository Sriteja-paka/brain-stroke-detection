...
def setup_models():
    ...
    try:
        ...
        # Load model with custom objects to avoid compilation warnings
        ultimate_model = tf.keras.models.load_model(
            main_model_file, 
            compile=False
        )
...
def main():
    ...
    if st.sidebar.button("🚀 Load AI Models", width='stretch'):
    ...
    with col1:
        ...
        if uploaded_file is not None:
            ...
            if detector.models_loaded:
                if st.button("🔍 Advanced Stroke Analysis", width='stretch', type="primary"):
