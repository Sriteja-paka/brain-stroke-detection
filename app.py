...
class BrainStrokeDetector:
    ...
    def generate_grad_cam(self, processed_img):
        """FIXED Grad-CAM using EfficientNet base model"""
        try:
            if not CV2_AVAILABLE:
                return None, None
            
            # Use EfficientNetV2S base model for Grad-CAM (not your custom model)
            base_model = EfficientNetV2S(weights='imagenet', include_top=False, 
                                       input_shape=(self.IMG_SIZE[0], self.IMG_SIZE[1], 3))
            
            # Find the last convolutional layer
            layer_name = None
            for layer in base_model.layers[::-1]:  # Search backwards
    ...
    def generate_shap_explanation(self, selected_features):
        ...
        try:
            ...
            background = np.random.rand(10, selected_features.shape[1]) * 0.1
            ...
            shap_values = explainer.shap_values(selected_features, nsamples=10)
            ...
            if len(stroke_shap_values.shape) > 1:
                stroke_shap_values = stroke_shap_values[0]  # Take first sample
            
            # Get top features
            top_n = min(10, len(stroke_shap_values))
    ...
    def predict_image(self, processed_img):
        """REAL PREDICTION with working Grad-CAM & SHAP"""
        try:
            if not self.models_loaded:
                return None
