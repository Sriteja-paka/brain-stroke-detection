...
class BrainStrokeDetector:
    ...
    def predict_image(self, processed_img):
        ...
        try:
            ...
            st.info(f"📊 Raw features extracted: {features.shape}")
            ...
            # DEBUG: Show selected features info
            st.info(f"🎯 Selected features (GWO): {selected_features.shape}")
            
            # 4. Main model prediction ONLY - NO BiLSTM
            status_text.text("🤖 Running main model...")
            
            # DEBUG: Show model input shape
            st.info(f"🧠 Model input shape: {selected_features.shape}")
            ...
            st.info(f"🎯 Predicted class: {final_class} (0=Normal, 1=Stroke)")
            ...
            st.info(f"🧠 Stroke probability: {stroke_prob:.4f}")
            st.info(f"✅ Normal probability: {normal_prob:.4f}")
...
def show_file_setup_page():
    ...
    st.info("""
    💡 **This version uses EXACT SAME preprocessing and feature extraction as your Colab code!**
    - Only difference: No BiLSTM model (main model only)
    - Added debugging info to verify predictions
