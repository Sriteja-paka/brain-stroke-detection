...
class BrainStrokeDetector:
    ...
    def generate_heatmap(self, brain_mask, stroke_confidence):
        ...
        # VERY LOW CONFIDENCE STROKE (<10%) - SHOW NO STROKE REGIONS (NORMAL BRAIN)
        if stroke_confidence < 0.1:
            # Return completely empty heatmap for normal brain
            return heatmap
        
        # LOW CONFIDENCE STROKE (10%-30%) - Show minimal subtle indications
        elif stroke_confidence < 0.3:
            # Only show very subtle indications for low confidence
            if stroke_confidence > 0.1:
                # Single very subtle region
    ...
    def generate_shap_analysis(self, image, stroke_confidence):
        ...
        try:
            with st.spinner("🔍 Performing SHAP feature analysis..."):
                ...
                # FIXED: Properly formatted f-string without line break
                ax1.set_title(f'Top {top_n} Most Influential Features (Stroke Confidence: {stroke_confidence*100:.1f}%)', 
                        ...
                        if processed_img is not None:
                            ...
                            if stroke_confidence is not None:
                                ...
                                with col2:
                                    ...
                                    # Big confidence badge with new category for <10%
                                    if confidence_percentage > 70:
                                        ...
                                        risk_class = "no-risk"
                                        risk_text = "🔵 NO STROKE DETECTED"
                                        recommendation = "Brain appears normal"
                                    ...
                                    # Heatmap overlay - Different colormap for normal brain
                                    if confidence_percentage < 10:
                                        # Show healthy brain with cool colors
                                        im = ax2.imshow(heatmap, cmap='cool', alpha=0.7)
                                        ax2.imshow(processed_img[:,:,0] if len(processed_img.shape)==3 else processed_img, cmap='gray', alpha=0.3)
                                        ax2.set_title('✅ Healthy Brain - No Stroke Regions', fontweight='bold')
                                        plt.colorbar(im, ax=ax2, fraction=0.046, pad=0.04, label='Brain Tissue')
                                    else:
                                        # Show stroke suspicion with hot colors
                                    ...
                                    # Special message for healthy brain
                                    if confidence_percentage < 10:
                                        st.markdown("""
                                        <div class="healthy-brain">
                                        <h4>🎉 EXCELLENT NEWS - HEALTHY BRAIN DETECTED!</h4>
                                        <ul>
                                            <li><b>No stroke regions identified</b> in the CT scan</li>
                                            <li>Brain tissue appears normal and healthy</li>
                                            <li>AI analysis shows very low stroke probability</li>
                                    ...
                                    with col_insight1:
                                        if confidence_percentage < 10:
                                            ...
                                            **🔍 Key Findings:**
                                            ...
                                            - Healthy brain anatomy confirmed
                                            """)
                                        else:
                                            st.markdown("""
                                            **🔍 Key Findings:**
                                    ...
                                    with col_insight2:
                                        if confidence_percentage < 10:
                                            ...
                                            **💡 Next Steps:**
                                            ...
                                            - Annual neurological screening recommended
                                            """)
                                        else:
                                            st.markdown("""
                                            **💡 Next Steps:**
